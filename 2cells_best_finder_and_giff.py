...
for k in range (N):             #Now we plot all the results for the N best cells combinations
    ...
    fig.suptitle('Rainbow analysis ' + BlueCell_device_name + ' (Blue cell) '+ BlueCells_names[i] +'/' + RedCell_device_name + '(Red cell) '+RedCells_names[j], fontsize=12)
    ...
    ax1.set_title('Efficiency vs Dividing Wavelength. IoBC(%)='+ str(format(IoBC[n],'.4f')))
    ax1.legend(loc='upper right')
    ax1.set(xlabel='Dividing Wavelength /nm', ylabel='PCE /%')
    filename = ('/Best combination_'+str(k+1)+'.png')
    Save_Directory = os.path.join(os.path.split(BlueCell_Files_Directory)[0],'Rainbow analysis (2 cells)', BlueCell_device_name +' + ' + RedCell_device_name, '15 best cells combinations')
...
for j in range (len(RedCell_megaarray[0,0,:])):
    for i in range (len(BlueCell_megaarray[0,0,:])):             #Now we plot all the results for the N best cells combinations
        ...
        ax1.plot(xblue, yblue, color='tab:blue', label = BlueCells_names[i] + ' Blue Cell' )
        ax1.plot(xtot, ytot, color='tab:green', label = 'Red cell + Blue Cell')
        ax1.set_title('Efficiency vs Dividing Wavelength. IOBC(%)='+str(format(IoBC[nn[0]],'.4f')))
        ax1.legend(loc='upper right')
        ax1.set_ylim(0,ymax*1.05)
        ax1.set(xlabel='Dividing Wavelength /nm', ylabel='Efficiency /%')             
        Save_Directory = os.path.join(os.path.split(BlueCell_Files_Directory)[0],'Rainbow analysis (2 cells)', BlueCell_device_name +' + ' + RedCell_device_name, 'GIFs')
        if not os.path.exists(Save_Directory):
            os.makedirs(Save_Directory)
    ...
    # Save GIF        
    exportname = Save_Directory+'/'+RedCell_device_name +'_'+ RedCells_names[j]+'_'+ BlueCell_device_name +'.gif'
